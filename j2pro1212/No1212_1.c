...
void disp_array_max_attach(int*,char name[][NAME],int*,int*,double*,double*);
...
int main(void)
{
  ...
  disp_array_max_attach(no,name,hp,mp,attack,defence);
...
void disp_array_max_attach(int* no,char name[][NAME],int* hp,int* mp,double* attack,double* defence)
{
  int i;
  int max_index = 0;
  for(i=0;i<PLAYER;i++)
  {
    if(attack[max_index]<attack[i])
    {
      max_index = i;
    }
  }
